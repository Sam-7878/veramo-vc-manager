import { IKey, RequireOnly, IIdentifier, VerifiableCredential } from '@veramo/core'
import { AbstractKeyStore } from '@veramo/key-manager'
import { AbstractPrivateKeyStore, ManagedPrivateKey } from '@veramo/key-manager'
import { AbstractDIDStore } from '@veramo/did-manager'
import { v4 as uuidv4 } from 'uuid'
import { VCStateVeramo, State, Wallet } from '../types/ISnap'
...
export class SnapDIDStore extends AbstractDIDStore {
  ...
  async get({ did, alias, provider }: { did: string; alias: string; provider: string }): Promise<IIdentifier> {
    let vcState = await getVCState()
    if (did && !alias) {
      if (!vcState.identifiers[did]) throw Error(`not_found: IIdentifier not found with did=${did}`)
      return vcState.identifiers[did]
    } else if (!did && alias && provider) {
      for (const key of Object.keys(vcState.identifiers)) {
        if (vcState.identifiers[key].alias === alias && vcState.identifiers[key].provider === provider) {
